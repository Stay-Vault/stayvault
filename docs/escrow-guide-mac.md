# StayVault エスクロー実装手順書（Mac・Claude Code版）

最終更新: 2026年9月25日（Anchor 1.2.0 に合わせて全面改訂）

**対象読者**: Mac で、1人で実装を進める人。同じ人が Windows でも作業し、区切りごとに端末を切り替える。Windows 側の手順は `escrow-guide-windows.md` にある。

**前提**: Mac のセットアップは完了している。Rust 1.90.0 / Solana CLI 4.1.2 / Anchor 1.2.0 / Node.js 22.23.2 / Yarn 1.22.22 が入り、`~/stayvault` にクローン済み。Program ID の鍵 `target/deploy/stayvault-keypair.json` は Mac で作った元の鍵で、Windows にも同じものを置いてある。
...
Git に入らない鍵ファイルは3つある。どれも中身を Claude Code に表示させたり、チャットに貼ったりしない。

| ファイル | 中身 |
| --- | --- |
| `target/deploy/stayvault-keypair.json` | Program ID の鍵。Mac で作った元の鍵。Windows にも同じものがある |
| `~/.config/solana/id.json` | この端末の devnet 財布の鍵。Windows とは別のもの |
| `app/wallet-demo.json` | デモ用の親役・運営者役の鍵。ステップ4で作る |

作業ブランチは `feat/escrow` の1本だけにする。Claude Code が別のブランチを勧めてきても使わない。
...
コミットメッセージには、どのステップまで進んだかを書く。`git status` の一覧に `wallet`、`-keypair.json`、`-backup.json`、`id.json` のどれかが含まれていたら、`add` せずに止まる（D章）。画面のサーバーを動かしていたら `Control + C` で止める。

### Mac に着いたら（Windows から戻ってきたとき）
...
`git log -1` に、Windows で書いたコミットメッセージが出れば同期できている。`yarn install` と `anchor build` は、Windows 側で部品やコードが変わっていても揃えるため。何度実行しても害はない。
...
Claude Code は起動時に `CLAUDE.md` を読む。ステップ1で `CLAUDE.md` を差し替えるので、ステップ1のあとは一度 `/exit` で終了して起動し直す。

各ステップの「Claude Code に頼む場合」の文を貼れば、コマンドを実行してもらえる。コマンドの実行前に許可を求められたら、内容を読んでから許可する。ビルドやテストは許可してよい。`anchor deploy` と `node scripts/setup-demo.mjs` は devnet の SOL を使うので、「今後も許可」は選ばない。`lib.rs` や `stayvault.html` を書き換える提案は、差分を見てから許可する。

詰まったときは、エラーを要約せずそのまま貼り、どのステップで出たかを添える。Claude Code が Anchor 0.32 以前の書き方（`CpiContext::new(ctx.accounts.token_program.to_account_info(), ...)` や `@coral-xyz/anchor`）を提案してきたら、「CLAUDE.md の固定バージョンの節に従って、Anchor 1.2.0 で書いて」と指示し直す。
...
**端末: zip がある方。** Windows で済ませた場合は、B章の「着いたら」をやればこのステップは不要。

### 1-1. zip を展開する

Finder で「ダウンロード」フォルダの `stayvault-escrow.zip` をダブルクリックする。`~/Downloads/stayvault-escrow` フォルダができる。以前展開したフォルダが残っていたら、先にごみ箱へ移してから展開する（`stayvault-escrow 2` のような別名で展開されるのを防ぐため）。

途中で macOS が「“ターミナル”が“ダウンロード”フォルダ内のファイルにアクセスしようとしています」と出したら許可する。
...
```
~/Downloads/stayvault-escrow/docs/escrow-guide-mac.md のステップ1-2を実行して。CLAUDE.md は上書きする前に、今のリポジトリの CLAUDE.md との差分を見せて。
```
...
```
docs/escrow-guide-mac.md のステップ2を 2-1 から 2-5 まで順に実行して。
...
Anchor 1.2.0 の `anchor init` は、`lib.rs` のほかに `instructions/` や `state/` などを分けて作る。今回の `lib.rs` は1ファイルにまとまっているので、雛形の残りは使わない。残しておくと、Claude Code がそちらを編集してしまうことがある。
...
- `Authority:` のアドレスと `solana address` が違う → Windows に更新権限がある。ここで B章の「離れる前」をやり、デプロイは Windows で行う
- `Unable to find the account` などと出る → まだ一度もデプロイしていない。ここでデプロイすると、Mac が更新権限を持つ。それでよければ 3-2 へ
...
`Authority:` がこの端末の `solana address` と同じなら、Mac が更新権限を持っている。初めてのデプロイだった場合は、次の2つを必ずやる。

1. `CLAUDE.md` の「更新権限のある財布: 未定」の行を、`- 更新権限のある財布: Mac（~/.config/solana/id.json、アドレス <solana address の値>）` に書き換える
...
```
docs/escrow-guide-mac.md のステップ4を実行して。setup-demo.mjs を実行する前に solana balance の結果を見せて止めて。
```
...
`-> app/wallet-demo.json を書き出しました` と出れば成功。このファイルは Git に入らないので、Windows でも画面を動かすなら Windows でもこのステップを実行する。
...
❌ が出たら、そこで止まる。その行と、直前に出たログをそのまま Claude Code に見せる。未検証のコードなので、ここで問題が見つかるのは想定内だ。`lib.rs` を直したら、ステップ3のデプロイをやり直してから、このステップを再実行する。
...
サーバーは起動したままになるので、Claude Code ではなく新しいターミナルのタブ（`Command + T`）で動かす。
...
ブラウザで `http://localhost:8000/stayvault.html` を開く。`stayvault.html` を Finder からダブルクリックで開くと `wallet-demo.json` を読めないので、必ずこの URL から開く。止めるときはサーバーのタブで `Control + C`。

### 5-2. デモの流れを通す

Chrome なら `Command + Option + J` で Console を開く。Safari なら「開発」メニューの「JavaScriptコンソールを表示」。`StayVault escrow ready:` と Program ID が出ていれば、画面はチェーンにつながっている。
...
**`git pull` で `CONFLICT`** → Mac と Windows で同じファイルを書き換え、片方を push し忘れた。どちらを残すかを決めてから、Claude Code に「このコンフリクトを解消して。残すのは○○の方」と頼む。

**`git status` に鍵ファイルが出る** → `add` しない。`-backup.json` はリポジトリの外に移すか消す。`wallet-demo.json` や `-keypair.json` が出るなら `.gitignore` が効いていないので、`cat .gitignore` の結果を Claude Code に見せる。

**`git pull` しても Windows の変更が来ない** → Windows で `git push` していないか、別のブランチにいる。`git branch` で `* feat/escrow` になっているか見る。
...
**`CpiContext` の引数の型が合わない（`expected Pubkey, found AccountInfo`）** → Anchor 0.32 の書き方が混ざっている。CLAUDE.md の固定バージョンの節にある3点に従うよう Claude Code に指示する。
...
**Program ID が合わないというエラー** → `solana address -k target/deploy/stayvault-keypair.json` を打つ。`GJet47eJ…` 以外が出たら鍵が違うので、バックアップから置き直す。`anchor keys sync` で直そうとしない。
...
**更新権限を Windows の財布に移したい** → 今の更新権限のある端末で次を実行する。新しいアドレスは Windows で `solana address` を打って出た値をそのまま貼る。1文字でも違うと誰も更新できなくなる。移したら CLAUDE.md の行も直す。

```bash
solana program set-upgrade-authority GJet47eJPYYAxHz5RFvxqVKv3n6d6uWZWPsRUSzjB5ZG --new-upgrade-authority <Windows の solana address> --skip-new-upgrade-authority-signer-check --url devnet
```
...
**`python3` が見つからない** → `xcode-select --install` を実行する。

**Console に `StayVault escrow ready` が出ない** → `wallet-demo.json` か外部ライブラリ（esm.sh）の読み込みで止まっている。赤いエラーを Claude Code に貼る。
...
- **push しないまま Windows に移る** → Windows は古い状態から作業を始め、あとで `CONFLICT` になる。
- **`git pull` せずにデプロイする** → Windows で直したコードが反映されないまま devnet に載る。
- **Claude Code の「バージョンを上げれば直る」「下げれば直る」提案を受け入れる** → Windows とずれて、片方だけビルドが通らなくなる。
...
- **鍵ファイルの中身を表示させる、コミットする** → devnet でも、その鍵は二度と使わず作り直す。
...
詰まった箇所は D章に1行ずつ足していく。Windows 版の手順書にも同じ項目を足しておくと、どちらの端末で詰まってもすぐ引ける。
